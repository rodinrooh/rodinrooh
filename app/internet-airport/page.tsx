...
function Tile({ ch, w, h, fs, color }: { ch: string; w: number; h: number; fs: number; color: string }) {
  return (
    <div style={{
      width: w, height: h, flexShrink: 0,
      background: `linear-gradient(to bottom, ${TILE_TOP} 50%, ${TILE_BTM} 50%)`,
      borderRadius: 4, border: '1px solid #0c0c0c',
      boxShadow: '0 2px 5px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.04)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: '50%', height: 1.5, background: CREASE, zIndex: 2, pointerEvents: 'none' }} />
      {ch ? (
        <span style={{ fontSize: fs, fontWeight: 500, color, lineHeight: 1, position: 'relative', zIndex: 3, letterSpacing: 0, textTransform: 'uppercase' }}>{ch}</span>
      ) : null}
    </div>
  )
}
...
function Colon({ fs, color }: { fs: number; color: string }) {
  return <span style={{ color, fontSize: fs, fontWeight: 700, lineHeight: 1, flexShrink: 0, paddingBottom: 3 }}>:</span>
}

function SlotRow({ text, w, h, fs, color, slots, gap = 3, rightAlign = false }: {
  text: string; w: number; h: number; fs: number; color: string; slots: number; gap?: number; rightAlign?: boolean
}) {
...
export default function Page() {
  ...
  const liveRows = useMemo(() => search ? results : domains, [search, results, domains])
  ...
  if (searching) {
    ...
    countLabel = search ? `${filteredLeader.length} MATCHES` : `TOP ${leaderboard.length}`
  } else if (search && rCount !== null) {
    countLabel = rCount > results.length ? `SHOWING ${results.length.toLocaleString()} OF ${rCount.toLocaleString()}` : `${rCount.toLocaleString()} MATCHES`
  } else {
  ...
  return (
    <main style={{ background: PAGE_BG, minHeight: '100vh', fontFamily: 'Inter, -apple-system, sans-serif' }}>
      {showIntro && (
        <div onClick={dismissIntro} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.82)', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#161616', border: '1px solid #2a2a2a', borderRadius: 10, padding: '36px 40px 28px', maxWidth: 460, width: '92%', boxShadow: '0 32px 80px rgba(0,0,0,0.9)' }}>
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <div style={{ color: '#888', fontSize: 12, lineHeight: 1.7 }}>A live feed of every single domain name registered on the internet. New domains are revealed continuously throughout the day.</div>
            </div>
            <div style={{ marginBottom: 22 }}>
              <div style={{ color: '#f5f5f5', fontSize: 10, letterSpacing: '0.2em', fontWeight: 700, marginBottom: 8 }}>WHERE IS THE DATA FROM?</div>
              <div style={{ color: '#888', fontSize: 12, lineHeight: 1.7 }}>Domain registration data is sourced from <span style={{ color: '#ccc' }}>WHOIS Daily</span>, which aggregates newly registered domains from registrars worldwide. Around <span style={{ color: '#ccc' }}>70,000+</span> new domains are registered each day.</div>
            </div>
            <div style={{ marginBottom: 28 }}>
              <div style={{ color: '#f5f5f5', fontSize: 10, letterSpacing: '0.2em', fontWeight: 700, marginBottom: 8 }}>HOW DOES IT WORK?</div>
              <div style={{ color: '#888', fontSize: 12, lineHeight: 1.7 }}>Each row is a domain that just got registered, and landed in the world of the Internet. They scroll in like an airport arrivals board. Every domain name gets a score from 0–100 based on how good it is. <span style={{ color: '#60b878' }}>Great</span> means it&apos;s great, <span style={{ color: '#c0c0c0' }}>Mid</span> means it&apos;s average, and <span style={{ color: '#e07820' }}>Trash</span> means it&apos;s the worst domain name ever.</div>
            </div>
            <button onClick={dismissIntro} style={{ background: '#f0b020', color: '#111', border: 'none', borderRadius: 4, padding: '11px 24px', fontSize: 10, letterSpacing: '0.22em', fontWeight: 700, cursor: 'pointer', width: '100%', marginBottom: 16 }}>
              HEAD TO ARRIVALS
            </button>
            <div style={{ textAlign: 'center', color: '#3a3a3a', fontSize: 10, letterSpacing: '0.08em' }}>
              By{' '}
              <a href="https://rodinrooh.com" target="_blank" rel="noopener noreferrer" style={{ color: '#555', textDecoration: 'none' }}
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 20, background: HDR_BG, borderBottom: '2px solid #181818', boxShadow: '0 4px 16px rgba(0,0,0,0.6)' }}>
        <div style={{ maxWidth: MAX_W, margin: '0 auto', padding: '14px 40px 0', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 14, right: 40, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10 }}>
            <BigClock now={now} />
            <div style={{ color: '#f5f5f5', fontSize: 9, letterSpacing: '0.2em', textAlign: 'right', lineHeight: 1.7 }}>{blurb}</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <div style={{ textAlign: 'left', marginBottom: 12 }}>
              <div style={{ color: '#f5f5f5', fontSize: 9, letterSpacing: '0.32em', marginBottom: 5 }}>INTERNATIONAL ARRIVALS</div>
              <div style={{ color: '#f0b020', fontSize: 26, letterSpacing: '0.12em', fontWeight: 600, lineHeight: 1 }}>INTERNET AIRPORT</div>
              <div style={{ color: '#f5f5f5', fontSize: 9, letterSpacing: '0.22em', marginTop: 5 }}>{countLabel}</div>
            </div>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="SEARCH ARRIVALS..."
              style={{ display: 'block', width: '100%', maxWidth: 480, background: 'transparent', border: 'none', borderBottom: '1px solid #3a3a3a', outline: 'none', color: '#bbb', fontSize: 11, letterSpacing: '0.16em', padding: '6px 0 8px', fontFamily: 'inherit', textAlign: 'left' }}
            />
            <div style={{ display: 'flex' }}>
              {(['live', 'top'] as const).map(t => (
                <button key={t} onClick={() => setTab(t)} style={{ background: 'transparent', border: 'none', borderBottom: tab === t ? '2px solid #888' : '2px solid transparent', color: tab === t ? '#f5f5f5' : '#666', fontFamily: 'inherit', fontSize: 9, letterSpacing: '0.28em', cursor: 'pointer', padding: '9px 24px 6px 0', fontWeight: tab === t ? 700 : 400 }}>
      ...
      <div style={{ maxWidth: MAX_W, margin: '0 auto', paddingTop: HDR_H + 20, paddingBottom: 80 }}>
        <div style={{ margin: '0 40px' }}>
          <div style={{ background: BOARD_BG, borderRadius: 8, border: '2px solid #1c1c1c', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: tab === 'live' ? LIVE_GRID : TOP_GRID, gap: '0 16px', padding: '10px 20px', position: 'sticky', top: HDR_H, zIndex: 9, background: BOARD_BG, borderBottom: '2px solid #1c1c1c' }}>
              {tab === 'live' ? (
                <><CH2>TIME</CH2><CH2>DEPARTURE</CH2><CH2>GATE</CH2><CH2 right>STATUS</CH2></>
              ) : (
                <><CH2>#</CH2><CH2>DEPARTURE</CH2><CH2>GATE</CH2><CH2 right>SCORE</CH2><CH2 right>STATUS</CH2></>
              )}
            </div>

            {tab === 'live' && liveRows.length === 0 && !searching && <Empty>{search ? 'NO MATCHES' : 'LOADING...'}</Empty>}
            {tab === 'live' && searching && <Empty>SEARCHING...</Empty>}
            {tab === 'live' && liveRows.map(d => {
              const isNew = newIds.has(d.id)
              return (
                <div key={d.id} className={`board-row${isNew ? ' flip-in' : ''}`}
                  onClick={() => window.open(`https://${d.domain}`, '_blank')}
                  style={{ display: 'grid', gridTemplateColumns: LIVE_GRID, gap: '0 16px', padding: '12px 20px', alignItems: 'center', background: '#1e1e1e', borderBottom: '2px solid #1c1c1c', cursor: 'pointer' }}>
                  <TimeTiles ts={d.shown_at} />
                  <SlotRow text={getSld(d.domain)} w={DW} h={DH} fs={DFS} color={isNew ? '#f8d060' : '#f0b020'} slots={DOM_SLOTS} />
                  <SlotRow text={getTld(d.domain)} w={TW} h={TH} fs={TFS} color='#f5f5f5' slots={TLD_SLOTS} gap={2} />
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}><StatusBadge status={getStatus(d.score)} /></div>
                </div>
              )
            })}

            {tab === 'top' && leaderLoading && <Empty>LOADING...</Empty>}
            {tab === 'top' && !leaderLoading && filteredLeader.length === 0 && <Empty>{search ? 'NO MATCHES' : 'NO DATA'}</Empty>}
            {tab === 'top' && filteredLeader.map(({ d, rank }) => (
              <div key={d.id} className="board-row"
                onClick={() => window.open(`https://${d.domain}`, '_blank')}
                style={{ display: 'grid', gridTemplateColumns: TOP_GRID, gap: '0 16px', padding: '12px 20px', alignItems: 'center', background: '#1e1e1e', borderBottom: '2px solid #1c1c1c', cursor: 'pointer' }}>
...
function CH2({ children, right }: { children: React.ReactNode; right?: boolean }) {
  return <div style={{ color: '#f0b020', fontSize: 9, letterSpacing: '0.28em', fontWeight: 700, textAlign: right ? 'right' : 'left' }}>{children}</div>
}
...
function StatusBadge({ status }: { status: 'GREAT' | 'MID' | 'TRASH' }) {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}`, fontSize: 9, letterSpacing: '0.18em', fontWeight: 700, padding: '5px 0', borderRadius: 3, whiteSpace: 'nowrap', display: 'inline-block', width: 72, textAlign: 'center' }}>{status}</span>
  )
}

function Empty({ children }: { children: React.ReactNode }) {
  return <div style={{ color: '#333', fontSize: 11, padding: '48px', textAlign: 'center', letterSpacing: '0.18em', background: '#1e1e1e', borderBottom: '2px solid #1c1c1c' }}>{children}</div>
}
