...
| `/waypoints` | `app/waypoints/` | Every Name in the Sky — full-screen canvas map of the US with all 69k FAA waypoints, searchable (static JSON, no storage) |
...
- **waypoints**: No storage, no env vars, no runtime API. `scripts/build-waypoints-data.mjs` (run manually, like the payroll script) downloads the FAA 28-Day NASR Subscription FIX CSV (free, no auth: `https://nfdc.faa.gov/webContent/28DaySub/extra/<DD>_<Mon>_<YYYY>_FIX_CSV.zip`), unzips `FIX_BASE.csv` with `adm-zip`, parses with `csv-parse`, keeps 3–5-letter alpha `FIX_ID`s with their decimal lat/lon, and writes four static files into `/public`: `waypoints.json` (full searchable set, ~2.2 MB), `waypoints-weird.json` (curated highlights + auto-detected real-word names via `word-list`), `waypoints-stats.json` (per-state counts / totals), and `us-states-10m.json` (us-atlas TopoJSON outline, downloaded at build). Bump `EFFECTIVE_DATE` in the script when the 28-day cycle rolls. The page (`app/waypoints/page.tsx`) is one full-screen map + a rounded search + a count bubble — no scroll sections. The map (`app/waypoints/components/WaypointMap.tsx`) is a **canvas US map**: a filled `geoAlbersUsa` (d3-geo) landmass (white land, blue water, visible state borders) with all 69k waypoints as navy dots on top, d3-zoom pan/zoom, d3-quadtree hover hit-testing, d3-transition to ease to a searched dot. No map tiles, no API key, no env vars — 100% client-side static. (`waypoints-weird.json`/`waypoints-stats.json` are still emitted by the build but currently unused. The ~618 grid points in the Gulf with `STATE_CODE=OG` are the real FAA Gulf of Mexico offshore helicopter grid, not a bug. An earlier Apple MapKit version was dropped: a 69k-point canvas overlay on MapKit's WebGL hung the page.)
