...
| `/waypoints` | `app/waypoints/` | Every Name in the Sky — all 69k FAA waypoints on a clean dark canvas map of the US, searchable (static JSON, no storage) |
...
- **waypoints**: No storage, no env vars, no runtime API. `scripts/build-waypoints-data.mjs` (run manually, like the payroll script) downloads the FAA 28-Day NASR Subscription FIX CSV (free, no auth: `https://nfdc.faa.gov/webContent/28DaySub/extra/<DD>_<Mon>_<YYYY>_FIX_CSV.zip`), unzips `FIX_BASE.csv` with `adm-zip`, parses with `csv-parse`, keeps 3–5-letter alpha `FIX_ID`s with their decimal lat/lon, and writes four static files into `/public`: `waypoints.json` (full searchable set, ~2.2 MB), `waypoints-weird.json` (curated highlights + auto-detected real-word names via `word-list`), `waypoints-stats.json` (per-state counts / totals), and `us-states-10m.json` (us-atlas TopoJSON outline, downloaded at build). Bump `EFFECTIVE_DATE` in the script when the 28-day cycle rolls. The viz (`app/waypoints/components/WaypointMap.tsx`) is a **canvas dark map**: a filled `geoAlbersUsa` (d3-geo) landmass of the US with all 69k waypoints drawn as dots on top, d3-zoom pan/zoom, a d3-quadtree for hover hit-testing, and d3-transition to ease to a searched dot. No map tiles, no API key, no env vars — 100% client-side static. (An earlier Apple MapKit version was dropped: compositing a 69k-point canvas overlay on MapKit's WebGL hung the page.)
